...
double aPBC(double Loc, int Nroot){
	if(Loc > dis * (Nroot + 1)){
		Loc -= dis * (Nroot + 1);
	} else if(Loc < 0){
		Loc += dis * (Nroot + 1);
...
int main(){
	...
	cout << "This is a PBC version" << endl;
					...
					for(int time = 0; time < nsteps; time ++){
						for(i = 0; i < n1; i ++){
							if(Mstatus[i]>0){
								...
								Mmindis[i] -= 2*moment;
								MlocX[i] = aPBC(MlocX[i], nroot);
								MlocY[i] = aPBC(MlocY[i], nroot);
							}
						}
						for(i = 0; i < n1; i ++){
							if(Fstatus[i]>0){
								...
								FlocX[i] = aPBC(FlocX[i], nroot);
								FlocY[i] = aPBC(FlocY[i], nroot);
							}
						}
